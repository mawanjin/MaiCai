    ...
    header.beginRefreshingBlock = ^(MJRefreshBaseView *refreshView) {
        ...
        if (self.segmentedControl.selectedSegmentIndex == 0) {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                NSMutableArray* newData = [[MCVegetableManager getInstance]getCollectionListByPage:page Pagesize:pageSize Recipe:true UserId:user.userId];
                if (newData) {
                    if (self.recipes == nil) {
                        self.recipes = [[NSMutableArray alloc]init];
                    }
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                [self.healthList removeAllObjects];
                MCUser* user = (MCUser*)[[MCContextManager getInstance]getDataByKey:MC_USER];
                NSMutableArray* newData = [[MCVegetableManager getInstance]getCollectionListByPage:page Pagesize:pageSize Recipe:false UserId:user.userId];
                if (newData) {
